...
function intersect(x::MC{N}, y::MC{N}) where N
  if (MC_param.mu >= 1)
    ...
    return MC{N}(max_MC.cv, min_MC.cc, intersect(Intv(a),Intv(b)), max_MC.cv_grad, min_MC.cc_grad, (x.cnst && y.cnst))
  else
    # Convex terms
    if (x.cv >= y.cv)
      cv = x.cv
    ...
    return MC{N}(cv, cc, intersect(Intv(x),Intv(y)), cv_grad, cc_grad, (x.cnst && y.cnst))
  end
end

function intersect(x::MC{N}, y::IntervalType) where N
  if (MC_param.mu >= 1)
    max_MC = max(x,lo(y))   # used for convex
    min_MC = min(x,hi(y))   # used for concave

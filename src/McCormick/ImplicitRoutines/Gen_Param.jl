...
function GenExpansionParams(h::Function, hj::Function,
  ...
  p_mc::Vector{MC{np}} = MC{np}[MC{np}(pmid[i],pmid[i],sone,sone,IntervalType(P[i].lo,P[i].hi),false) for i=1:np]
  pref_mc::Vector{MC{np}} = copy(p_mc)
  aff_mc::Vector{MC{np}} = MC{np}[MC{np,V,T}(xa_mc[i].cv,xA_mc[i].cc,IntervalType(xa_mc[i].Intv.lo,xA_mc[i].Intv.hi),szero,szero,false) for i=1:nxi]

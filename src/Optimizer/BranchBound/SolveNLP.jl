...
function SolveNLP!(x::Optimizer)
  ...
  while (x.TerminationCheck(x))
    ...
    x.History.PreprocessTime[x.CurrentIterationCount] = x.PreprocessTimeHistory[x.CurrentIterationCount-1]+PreprocessTime
    redirect_stdout(TT)
    ...
    if (x.CurrentPreprocessInfo.Feasibility)
      ...
      x.History.LowerBound[x.CurrentIterationCount] = x.LowerBoundHistory[x.CurrentIterationCount-1]+LowerProblemTime
      ...
      while x.CutCondition(x)
        ...
        redirect_stdout()
        LowerProblemTime = @elapsed x.LowerProblem!(x,CurrentNode)
        x.History.LowerBound[x.CurrentIterationCount] = x.LowerBoundHistory[x.CurrentIterationCount]+LowerProblemTime
        x.History.LowerCount += 1
        redirect_stdout(TT)
        PrintResults!(x,true)
      end
      ...
      if (x.CurrentLowerInfo.Feasibility)
        if (~x.ConvergenceCheck(x))
          ...
          x.History.UpperTime[x.CurrentIterationCount] = x.UpperTimeHistory[x.CurrentIterationCount-1]+UpperProblemTime
          ...
          x.History.PostprocessTime[x.CurrentIterationCount] = x.PostprocessTimeHistory[x.CurrentIterationCount-1]+PostprocessTime
          PostSaveFlag = true
          redirect_stdout(TT)
        ...
        x.History.UpperBound[x.CurrentIterationCount] = x.UpperBoundHistory[x.CurrentIterationCount-1]
      end
      Fathom!(x)
    else
    ...
    (~UBDTempSaveFlag) && (x.History.UpperTime[x.CurrentIterationCount] = x.UpperTimeHistory[x.CurrentIterationCount-1])
    (~PostSaveFlag) && (x.History.PostprocessTime[x.CurrentIterationCount] = x.PostprocessTimeHistory[x.CurrentIterationCount-1])

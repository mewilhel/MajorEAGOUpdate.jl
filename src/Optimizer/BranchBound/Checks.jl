...
function DefaultTerminationCheck(x::Optimizer)
    ...
    t1 = length(x.Storage.Stack) > 0                           # stack is non-empty
    t2 = x.CurrentIterationCount < x.IterationLimit            # maximum iterations not exceeded
    t3 = length(x.Storage.Stack) < x.CurrentNodeCount          # maximum node number not exceeded
    t4 = (U - L) > x.AbsoluteTolerance                           # absolute tolerance satisfied
    t5 = (U - L) > abs(L)*x.RelativeTolerance || ~(L > -Inf)     # relative tolerance satisfied

    if t1 & t2 & t3 & t4 & t5
      return true
    else
      if (x.Verbosity == Verbosity2 || x.Verbosity == Verbosity3)
        if ~t1

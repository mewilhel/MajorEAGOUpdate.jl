...
function SolveImplicit(f, h, xl::Vector{Float64}, xu::Vector{Float64},
                       pl::Vector{Float64}, pu::Vector{Float64}, opt;
    ...
    build_lower_evaluator!(ImpLowerEval, h, np, nx, obj = f, constr = g,
                           state_jac = hj, user_sparse = user_sparsity)
    ...
    has_obj = (f == nothing)
    bnd_pair = NLPBoundsPair(-Inf,0.0)
